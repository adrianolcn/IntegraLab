  ...
  return (
    <div className="my-8 overflow-hidden rounded-2xl border border-borderSubtle bg-surface shadow-sm">
      ...
      <div className="grid grid-cols-1 gap-0 xl:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)]">
        <div className="grid grid-cols-1 gap-px bg-borderSubtle xl:grid-cols-2">
          <div className="bg-slate-950 p-5 text-slate-300">
            <div className="mb-4 flex items-center justify-between">
              ...
              <span className="rounded-full bg-sky-500/10 px-2.5 py-1 text-[11px] font-bold text-sky-300">
                {t('rrAnatomy.requestActor', 'Cliente')}
              </span>
            </div>

            <div className="rounded-xl border border-slate-800 bg-slate-900/80 px-3 py-3 font-mono text-sm">
            ...
            <div className={`mt-3 cursor-pointer rounded-xl border border-slate-800 px-3 py-3 transition-all ${requestActiveClass('requestHeaders', 'bg-white/0')}`} onClick={() => setActivePart('requestHeaders')}>
              <div className="break-all text-purple-300">{t('rrAnatomy.code.request.hostLabel')}: <span className="text-slate-400">{t('rrAnatomy.code.request.hostValue')}</span></div>
              <div className="break-all text-purple-300">{t('rrAnatomy.code.request.authorizationLabel')}: <span className="text-slate-400">{t('rrAnatomy.code.request.authorizationValue')}</span></div>
              <div className="break-all text-purple-300">{t('rrAnatomy.code.request.contentTypeLabel')}: <span className="text-slate-400">{t('rrAnatomy.code.request.contentTypeValue')}</span></div>
            </div>
            ...
            <div className={`mt-2 cursor-pointer overflow-hidden rounded-xl border border-transparent px-3 py-3 transition-all ${requestActiveClass('requestBody', 'bg-amber-500/10')}`} onClick={() => setActivePart('requestBody')}>
          ...
          <div className="bg-slate-950 p-5 text-slate-300">
            ...
            <div className={`cursor-pointer rounded-xl border border-slate-800 px-3 py-3 font-mono text-sm transition-all ${responseActiveClass('status', 'bg-emerald-500/10')}`} onClick={() => setActivePart('status')}>
            ...
            <div className={`mt-3 cursor-pointer rounded-xl border border-slate-800 px-3 py-3 transition-all ${responseActiveClass('responseHeaders', 'bg-white/0')}`} onClick={() => setActivePart('responseHeaders')}>
              <div className="break-all text-cyan-300">{t('rrAnatomy.code.response.contentTypeLabel')}: <span className="text-slate-400">{t('rrAnatomy.code.response.contentTypeValue')}</span></div>
              <div className="break-all text-cyan-300">{t('rrAnatomy.code.response.locationLabel')}: <span className="text-slate-400">{t('rrAnatomy.code.response.locationValue')}</span></div>
              <div className="break-all text-cyan-300">{t('rrAnatomy.code.response.requestIdLabel')}: <span className="text-slate-400">{t('rrAnatomy.code.response.requestIdValue')}</span></div>
            </div>

            <div className={`mt-4 cursor-pointer overflow-hidden rounded-xl border border-transparent px-3 py-3 transition-all ${responseActiveClass('responseBody', 'bg-emerald-500/10')}`} onClick={() => setActivePart('responseBody')}>
